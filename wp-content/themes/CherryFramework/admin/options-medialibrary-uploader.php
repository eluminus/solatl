<?php
// ---- Added by patch: safe count for PHP 7.2+ (handles arrays, Countable, and objects) ----
if (!function_exists('of_safe_count')) {
    function of_safe_count($v) {
        if (is_array($v) || $v instanceof Countable) { return \count($v); }
        if (is_object($v)) { return \count(get_object_vars($v)); }
        return 0;
    }
}
// ---- End safe count helper ----
