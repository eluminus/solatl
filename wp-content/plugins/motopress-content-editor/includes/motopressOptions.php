...
//    register_setting('motopressCEOptionsFields', 'motopressCEOptions'/*, 'plugin_options_validate'*/);
    ...
    register_setting('motopressCEGoogleGeocodeApiKeyFields', 'motopressContentEditorOptions');
    add_settings_section('motopressCEGoogleGeocodeApiKeyFields', '', 'motopressCEGoogleGeocodeApiKeySecTxt', 'motopress_options', 'motopressCEGoogleGeocodeApiKeyFields');
    add_settings_field('motopressCEGoogleGeocodeApiKey', __("Google Maps API Key", 'motopress-content-editor'), 'motopressCEGoogleGeocodeApiKeyFields', 'motopress_options', 'motopressCEGoogleGeocodeApiKeyFields');
...
function motopressCECustomCSSFields() {
    ...
    if ( !mpceSettings()['wp_upload_dir_error'] ) {
        ...
        if ( is_writable(mpceSettings()['plugin_upload_dir_path']) ) {
            ...
            printf(__("Note: you are not able to edit custom css code because the directory %s not found or not writable.", 'motopress-content-editor'), mpceSettings()['plugin_upload_dir_path']);
        }
    }else {
        printf(__("Note: you are not able to edit custom css code because the directory %s not found or not writable.", 'motopress-content-editor'), mpceSettings()['wp_upload_dir']);
    }
}
...
function motopressCEGoogleFontsFields() {
    ...
    if (!isset($error['error'])) {
        ...
        echo '<p>' . sprintf(__("Use this form to add <a href='https://www.google.com/fonts'>Google Fonts</a> to %s. You can find these fonts at the Style panel of the text objects. Press <b>Save</b> button at the bottom of the page to apply your changes.", 'motopress-content-editor'), mpceSettings()['brand_name']) . '</p><br/>';
...
function motopressCEGoogleGeocodeApiKeySecTxt() {}
function motopressCEGoogleGeocodeApiKeyFields() {
    $googleApiKey = get_option('motopress-ce-google-geocode-api-key', '');
	echo '<input type="text" name="google_geocode_api_key" value="' . esc_attr($googleApiKey) . '" class="regular-text" />';
    echo '<p class="description">' . sprintf( __('<a %1$s>Follow this link</a> to obtain your API key. In short, you need to:<br/>1. Enable "Google Maps JavaScript API" and "Google Maps Geocoding API" in <a %2$s>Google Developers Console</a><br/>2. Paste it to this field and save',
...
function motopressCEHideFields() {
    ...
    echo '<label><input type="checkbox" name="hide_options"' . $checked . '>' . sprintf(__("Hide %s Settings on subsites", 'motopress-content-editor'), mpceSettings()['brand_name']) . '</label><br>';
}
...
function motopressCESettingsSave() {
	...
	if ($pluginId === mpceSettings()['plugin_short_name']) {
		if (!empty($_POST)) {
          ...
          //Google Geocode Api Key Setting
          if (isset($_POST['google_geocode_api_key'])) {
            $googleApiKey = sanitize_text_field($_POST['google_geocode_api_key']);
            update_option('motopress-ce-google-geocode-api-key', $googleApiKey);
          }
            ...
            <tbody>
                <tr valign="top">
                    <th scope="row" valign="top">
                        <?php echo __("License Key", 'motopress-content-editor') . " (" . mpceSettings()['license_type'] . ")"; ?>
                    </th>
                    <td>
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            ...
                            <?php
                                        ...
                                        <a href="<?php echo mpceSettings()['renew_url']; ?>" class="button-secondary" target="_blank"><?php echo __("Renew License", 'motopress-content-editor'); ?></a>
                            <?php
                                    }
                                }
                            ?>
