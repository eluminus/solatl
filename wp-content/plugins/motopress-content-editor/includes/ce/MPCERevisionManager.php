...
class MPCERevisionManager
{
    ...
    private function getRevisions($postID = 0, $queryArgs = array(), $parseResult = true)
    {
        ...
        foreach ($posts as $revision) {
            $date = date_i18n(
                _x('M j @ H:i', 'revision date format', 'motopress-content-editor'),
            ...
            $revisions[] = array(
                ...
                'date'     => sprintf(__('%1$s ago (%2$s)', 'motopress-content-editor'), $humanTime, $date),
                'type'     => $type,
    ...
    public function onRevisionDataRequest()
    {
        ...
        if (!$id || !wp_is_post_revision($id)) {
            motopressCESetError(__('Invalid Revision', 'motopress-content-editor'));
        }

        if (!MPCEAccess::getInstance()->hasAccess($id)) {
            motopressCESetError(__('You have no access to this revision', 'motopress-content-editor'));
        }
    ...
    public function onDeleteRevisionRequest()
    {
        ...
        if (!$id) {
            motopressCESetError(__("You must set the id", 'motopress-content-editor'));
        }
        ...
        if ($deleted && !is_wp_error($deleted)) {
            ...
            motopressCESetError(__("Cannot delete this Revision", 'motopress-content-editor'));
        }
    }
