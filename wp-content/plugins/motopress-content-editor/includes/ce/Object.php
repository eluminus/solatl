...
class MPCEObject extends MPCEElement {
    ...
    public function setCloseType($closeType) {
        if (is_string($closeType)) {
            ...
            if (!empty($closeType)) {
                $closeType = htmlspecialchars($closeType);
                if ($closeType === self::SELF_CLOSED || $closeType === self::ENCLOSED) {
                    $this->closeType = $closeType;
                } else {
                    $this->addError('closeType', sprintf(__("Value should be one of these values: %s", 'motopress-content-editor'), implode(', ', array(self::SELF_CLOSED, self::ENCLOSED))));
                }
            } else {
                $this->addError('closeType', __("Value should not be empty", 'motopress-content-editor'));
            }
        } else {
            $this->addError('closeType', sprintf(__("Invalid argument type - %s", 'motopress-content-editor'), gettype($closeType)));
        }
    }
    ...
    public function setResize($resize) {
        if (is_string($resize)) {
            $resize = trim($resize);
            if (!empty($resize)) {
                $resize = htmlspecialchars($resize);
                if (
                    ...
                    $this->addError('resize', sprintf(__("Value should be one of these values: %s", 'motopress-content-editor'), implode(', ', array(self::RESIZE_NONE, self::RESIZE_HORIZONTAL, self::RESIZE_VERTICAL, self::RESIZE_ALL))));
                }
            } else {
                $this->addError('resize', __("Value should not be empty", 'motopress-content-editor'));
            }
        } else {
            $this->addError('resize', sprintf(__("Invalid argument type - %s", 'motopress-content-editor'), gettype($resize)));
        }
    }
    ...
    public function addParameter(array $parameter) {
        if (!empty($parameter)) {
            ...
            $this->addError('parameters', __("Value should not be empty", 'motopress-content-editor'));
        }
    }
	...
	public function addStyle(array $style) {
	   ...
	   if (!empty($style)) {
		   ...
		   $this->addError('styles', __("Value should not be empty", 'motopress-content-editor'));
	   }
   }
