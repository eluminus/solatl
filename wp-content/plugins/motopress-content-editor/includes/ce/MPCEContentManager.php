...
//				wp_redirect(get_edit_post_link($postID, false));
		...
		public function addFieldsToEditPostForm() {
			...
			if ( $this->accessChecker->hasAccess( $post ) ) {
				...
				$wpTitle         = __( "Text Editor", 'motopress-content-editor' );
				$mpceTitle       = __( "Visual Builder", 'motopress-content-editor' );

				// Visual Editor button
				$postStatus   = get_post_status( $postID );
				$CEButtonText = apply_filters( 'mpce_button_text', __( "Open Visual Builder", 'motopress-content-editor' ) );
				?>
		...
		function wpPostRevisionFields( $fields ) {
			$fields[ self::CONTENT_EDITOR_ID ] = __( "Visual Builder", 'motopress-content-editor' );
