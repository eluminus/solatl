...
// ---- Added by patch: safe unserialize for PHP 7.2+ ----
if (!function_exists('cherry_safe_unserialize')) {
    function cherry_safe_unserialize($str) {
        if (!is_string($str) || $str === '') return null;
        if (function_exists('is_serialized') && !is_serialized($str)) {
            $decoded = base64_decode($str, true);
            if ($decoded === false || (function_exists('is_serialized') && !is_serialized($decoded))) {
                return null;
            }
            $str = $decoded;
        }
        $result = @cherry_safe_unserialize($str, ['allowed_classes' => false]);
...
function cherry_plugin_remote_query( $atts ) {
	...
	$response = cherry_safe_unserialize( $response['body'] );
	if ( $response == null ) {
		return;
	}
